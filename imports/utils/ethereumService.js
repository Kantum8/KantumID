...
var ethereumService = {
  initialize: function(callback) {
    ...
    // DEV env
    web3.eth.getTransaction('0x693fa977b2c65872799a5f99ce4d7ee51e6c9ab6f918d489fb9cacddc25f14bf', function(error, result) {
      if(result == null) {
...
var ethereumService = {
  initialize: function(callback) {
    ...
    eth.getTransaction('0xcf5240b6ed8ff000f3f537ab16839ff4ead6dffa19a6a4a4850dcc34d24d4b99', function(error, result) {
      if(result == null) {
        console.log("Web3 error: " + error);
        return callback(false);
      } else {
        emailContract = web3.eth.contract(contractAbi).at(contractAddress);
        callback(true);
      }
    });
  },
  ...
  // Check if current MetaMask user has already registered an account
  checkIfUserExists: function(callback) {
    var broadcastPublicKeyEvent = emailContract.BroadcastPublicKey({addr: web3.eth.accounts[0]}, {fromBlock: 0, toBlock: 'latest'});
...
// Emit new email contract event
  writeEmail: function(toAddress, ipfsHash, inReplyToIpfsHash, callback) {
    ...
    emailContract.sendEmail(toAddress, ipfsHash, inReplyTo, inReplyToIpfsHash, function(error, result) {
      if (error) {
        console.log("Could not execute sendEmail() contract function!" + error);
        return callback("Could not execute sendEmail() contract function!" + error, null);
      }
